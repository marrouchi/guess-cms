...
module.exports = {
  ...
  methods: {
    changelog: function(websiteUrl, callback) {
      ...
      request({
        ...
        if(error){
          callback({method: 'changelog', err: error}, null);
          return;
        }
    ...
    changelogV8: function(websiteUrl, callback) {
      ...
      request({
        ...
        // Something went wrong
        if(error){
          callback({method: 'changelogV8', err: error}, null);
          return;
        }
    ...
    nodeCss: function(websiteUrl, callback) {
      ...
      request({
        ...
        // Something went wrong
        if(error){
          callback({method: 'nodeCss', err: error}, null);
          return;
        }
  ...
  version: function(websiteUrl, callback){
    ...
    request({
      ...
      // Check version
      var regex = /Drupal ([0-9\.]+), [0-9]{4}-[0-9]{2}-[0-9]{2}/i;
      var match = body.match(regex);
      if(match && match.length == 2){
        callback(null, match[1]);
      }else{
        callback(new Error('Unable to get version in (/CHANGELOG.txt)'), null);       
      }
    });
  }
};